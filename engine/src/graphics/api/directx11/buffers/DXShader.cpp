...
void DXShader::CreateConstantBuffers() {
    ...
    HRESULT hr = m_device->CreateBuffer(&CB_desc, nullptr, &this->mShader_ctx_buffer);
    ...
    XV_LOG_DEBUG("{} : {} ", "DX11_SHADER", "Created Constant Buffers");
}
...
void DXShader::PassMat4( glm::mat4 mat ) {
    ...
    m_context->Map(this->mShader_ctx_buffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &map_out );

    memcpy(map_out.pData, &shader_data, sizeof(shader_data));

    m_context->Unmap(this->mShader_ctx_buffer.Get(), 0);
    m_context->VSSetConstantBuffers(0, 1, this->mShader_ctx_buffer.GetAddressOf());
}
...
void DXShader::CompileShaders( const wchar_t* hlsl_shader_source ) {
    ...
    hr = m_device->CreateVertexShader(this->mVS_shader_buffer->GetBufferPointer(), this->mVS_shader_buffer->GetBufferSize(), NULL, &this->mVS_shader);

    if (FAILED(hr)) {
        XV_LOG_ERROR("{} : {}", "DirectX11 Shader Error", "Failed to Create VS on DEFAULT.hlsl");
    }

    hr = m_device->CreatePixelShader(this->mPS_shader_buffer->GetBufferPointer(), mPS_shader_buffer->GetBufferSize(), NULL, &this->mPS_shader);

    if (FAILED(hr)) {
        XV_LOG_ERROR("{} : {}", "DirectX11 Shader Error", "Failed to Create PS on DEFAULT.hlsl");
    }

    XV_LOG_DEBUG("{} : {}", "DX11_SHADER", "Compiled HLSL Shaders");

    m_context->VSSetShader(this->mVS_shader.Get(), 0, 0);
    m_context->PSSetShader(this->mPS_shader.Get(), 0, 0);
}